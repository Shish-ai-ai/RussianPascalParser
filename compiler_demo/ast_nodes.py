...
from compiler_demo.my_semantic import TYPE_CONVERTIBILITY, BIN_OP_TYPE_COMPATIBILITY, BinOp, \
...
class ArrayDeclNode(StmtNode):
    """Класс для представления в AST-дереве объявления массива"""

    def __init__(self, type_: TypeNode, name: IdentNode, size: ExprNode, size2: Optional[ExprNode] = None,
                 row: Optional[int] = None, col: Optional[int] = None, **props) -> None:
        super().__init__(row=row, col=col, **props)
        self.type = type_
        self.name = name
    ...
    @property
    def childs(self) -> Tuple[AstNode, ...]:
        return (self.type, self.name, self.size, *((self.size2,) if self.size2 else tuple()))

    def semantic_check(self, scope: IdentScope) -> None:
        self.type.semantic_check(scope)
        self.size.semantic_check(scope)
        if self.size.node_type != TypeDesc.INT:
            self.semantic_error(f'Размер массива должен быть целым числом, а не {self.size.node_type}')
        if self.size2:
            self.size2.semantic_check(scope)
            if self.size2.node_type != TypeDesc.INT:
                self.semantic_error(f'Размер массива должен быть целым числом, а не {self.size2.node_type}')
            scope.add_ident(IdentDesc(self.name.name, TypeDesc(self.type.type, is_array=True, dimensions=2)))
        else:
            scope.add_ident(IdentDesc(self.name.name, TypeDesc(self.type.type, is_array=True)))
        self.node_type = TypeDesc.VOID


class ArrayElemNode(ExprNode):
    """Класс для представления в AST-дереве элемента массива"""

    def __init__(self, name: IdentNode, index: ExprNode, index2: Optional[ExprNode] = None,
                 row: Optional[int] = None, col: Optional[int] = None, **props) -> None:
        super().__init__(row=row, col=col, **props)
        self.name = name
        self.index = index
    ...
    def __str__(self) -> str:
        return f'{self.name}[{self.index}]{f"[{self.index2}]" if self.index2 else ""}'

    @property
    def childs(self) -> Tuple[AstNode, ...]:
        return (self.name, self.index, *((self.index2,) if self.index2 else tuple()))

    def semantic_check(self, scope: IdentScope) -> None:
        self.name.semantic_check(scope)
        self.index.semantic_check(scope)
        if not self.name.node_type.is_array:
            self.semantic_error(f'{self.name.name} не является массивом')
        if self.index.node_type != TypeDesc.INT:
            self.semantic_error(f'Индекс массива должен быть целым числом, а не {self.index.node_type}')
        if self.index2:
            self.index2.semantic_check(scope)
            if self.index2.node_type != TypeDesc.INT:
                self.semantic_error(f'Индекс массива должен быть целым числом, а не {self.index2.node_type}')
